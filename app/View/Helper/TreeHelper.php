 ...
 * @lastModified         $Date: 2008/11/21 09:06:32 $
 *
...
class TreeHelper extends AppHelper
{
     ...
     * @var array
     */
    public $__typeAttributes = [];
    /**
     * typeAttributesNext property.
     *
     * @var array
     */
    public $__typeAttributesNext = [];
    /**
     * itemAttributes property.
     *
     * @var array
     */
    public $__itemAttributes = [];
    /**
     * helpers variable.
     *
     * @var array
     ...
     */
    public function generate($data, $settings = [])
    {
        ...
        if ($model === null) {
            $ext_239 = empty($view->params['models'][0]) ? '' : $view->params['models'][0];
            $model = Inflector::classify($ext_239);
        }
        $stack = [];
        if ($depth == 0) {
            if ($class) {
        ...
        foreach ($data as $i => $result) {
            ...
            /* Prefix */
            if ($__addType) {
                $typeAttributes = $this->__attributes($type, ['data' => $elementData]);
     ...
     */
    public function __attributes($rType, $elementData = [], $clear = true)
    {
        extract($this->__settings);
        if ($rType == $type) {
            ...
            if ($clear) {
                ...
                $this->__typeAttributesNext = [];
            }
        } else {
            $attributes = $this->__itemAttributes;
            $this->__itemAttributes = [];
            if ($clear) {
                $this->__itemAttributes = [];
            }
        }
    ...
    public function filter($menus, $myrole)
    {
        foreach ($menus as $k => $menu1) {
            if (in_array($myrole, $menu1['deny'])) {
                unset($menus[$k]);
            }
        }
